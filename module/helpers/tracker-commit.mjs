...
export async function postTrackerStatus(tracker) {
  ...
  const line = hasGoal
    ? game.i18n.format("ASTER.tracker.statusLineGoal", {
        name: tracker.name,
        value: tracker.value,
        goal: tracker.goal,
      })
    : game.i18n.format("ASTER.tracker.statusLine", { name: tracker.name, value: tracker.value });
  const done =
    hasGoal && tracker.value >= tracker.goal
      ? `<span class="tracker-status-done">${game.i18n.localize("ASTER.tracker.done")}</span>`
      : "";

  await ChatMessage.create({
    content: `<div class="aster-chat-card tracker-status-card">
      <header class="card-header"><div class="title"><div class="name">
        ${game.i18n.localize("ASTER.tracker.statusTitle")}
      </div></div></header>
      <div class="tracker-status-body">${line}${done}</div>
    </div>`,
    speaker: ChatMessage.getSpeaker({ alias: game.i18n.localize("ASTER.world.panelTitle") }),
  });
}
