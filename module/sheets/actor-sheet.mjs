...
export class AsterActorSheet extends HandlebarsApplicationMixin(ActorSheetV2) {
  ...
  async _prepareContext(options) {
    ...
    context.badstatusTips = AsterActorSheet.#buildStatusTooltips();
    ...
    context.inv = {
        ...
        ? {
            ...
            items: inBag.map((i) => {
              ...
              return {
                ...
                tooltip: AsterActorSheet.#buildItemTooltip(i, inBagLabel),
                w,
                h,
                x: i.system.grid?.x ?? 0,
                y: i.system.grid?.y ?? 0,
      ...
      allItems: [
        ...
        ...inStorage.map((i) => ({
          ...
          name: i.name,
          img: i.img,
          tooltip: AsterActorSheet.#buildItemTooltip(i, inStorageLabel),
          location: "storage",
  ...
  _prepareSpellList(context) {
    ...
    context.spells = spells.map((s) => {
      const formula = this.#formatFormula(s); // "녹+박식(12)"
      return {
        id: s.id,
        name: s.name,
        img: s.img,
        ...
        tooltip: AsterActorSheet.#buildSpellTooltip(s, formula),
        formula,
      };
    });
  }
  ...
  static #BADSTATUS_KEYS = ["injury", "biginj", "sleepy", "exhaustion", "hungry"];
   ...
   */
  static #buildStatusTooltips() {
    const tips = {};
    for (const key of AsterActorSheet.#BADSTATUS_KEYS) {
   ...
   * @returns {string|null}
   */
  static #buildItemTooltip(item, locationLabel) {
    const desc = (item.system.description ?? "").trim();
    if (!desc) return null;
    const icon = AsterActorSheet.#TOOLTIP_ICON[item.type] ?? "fa-book";
   ...
   * @returns {string|null}
   */
  static #buildSpellTooltip(spell, formula) {
