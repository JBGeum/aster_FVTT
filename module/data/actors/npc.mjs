...
export class NpcDataModel extends BaseActorModel {
  static defineSchema() {
    return {
      ...
      // N1 — 능력치 식 표현 (옵션 A). PC는 능력치 합산, NPC는 룰북 표처럼 *식*으로 표현.
      // 판정 시 new Roll(formula).evaluate()로 평가 (N3~N4). 빈 문자열이면 해당 판정 없음.
      ...
      // 현재 라운드 AP. N4에서 라운드 시작 시 apFormula로 굴려 자동 채움. N1은 수동 입력만.
      ap: new fields.SchemaField({
        value: new fields.NumberField({ initial: 0, integer: true, min: 0 }),
        max: new fields.NumberField({ initial: 0, integer: true, min: 0 }),
      }),

      // N1 — 상태이상 (PC와 동일 5종). DAMAGE_STATUSES 키 일치 → 회복/대미지 헬퍼가 NPC에도 동작.
