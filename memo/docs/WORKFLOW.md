...
> 마지막 갱신: R 트랙 완료(R1·R2 명세 완성) + F 트랙(룰 정합 마무리) · I 트랙(아이템 시트 정리) 자리잡기.
...
graph LR
    ...
    subgraph F["F. 룰 정합 마무리"]
        F1[F1 전투 종료 자동 회복 + 회복 차단]:::current
        F2[F2 추가 룰 정합 영역]:::future
    end
...
| **35** | **(예정)** | **F1_SPEC** | **F1 전투 종료 자동 회복 + 회복 효과 차단** | 🔵 다음 |
| 36 | (선택) | F2_SPEC | F2 추가 룰 정합 영역 (포만 0 패널티·피크닉 양도 등) | 🟦 후속 |
| 37 | (예정) | I1_SPEC | I1 기존 아이템 타입 시트·템플릿 점검·갱신 | 🟦 후속 |
| 38 | (예정) | T1_PACK_TOOLING_SPEC | T1 빌드·역추출 도구 (LevelDB 관리) | 🟨 시점 미정 |
...
| 순위 | 작업 | 근거 |
|---|---|---|
| 1 | **F1 전투 종료 자동 회복 + 회복 차단** | 룰북 537 명시. R 트랙의 자연 연장 |
| 2 | **F2 추가 룰 정합** (선택) | 포만 0 패널티·피크닉 양도 등. 룰북 명시 잔재 정리 |
| 3 | **I1 기존 아이템 타입 시트·템플릿 정리** | 실제 데이터 사용 패턴 기반 점검·갱신 |
| — | **검증 단계** — D31~D36 + R 트랙 + F·I 통합 체크리스트 진행 | 모든 기능 통합 회귀 |
| 4 | **H1 채팅 카드 일관성 통일** | 검증 후 최종 단계. D32 기반 |
| 5 | **H2 스크린샷 기반 시트 점검** | Foundry 실제 화면 첨부 |
| 6 | **H3 다크/라이트 테마 (선택)** | 두 테마에서 모두 정상 동작 |
| 7 | T1 도구 트랙 (시점 미정) | 필요 시점에 진행 |
...
- 전투 종료 시 행동불능 PC 자동 건강 1 회복 (D23 확장 또는 새 STEP)
- 전투 중 행동불능 PC에 회복 효과(applyHealHealth·R1 consumable 등) 차단

작업 자산:
- `_endCombat` hook 또는 D23 흐름 확장
- `applyHealHealth`에 *전투 중 + 건강 0* 분기 추가
- R1 useConsumable에 같은 분기

### F2 — 추가 룰 정합 (선택)

**범위**: 룰북 명시이지만 *미진행* 영역. 가치·분량 평가 후 진행:
- 포만 0 패널티 정확화 (룰북 493 — 판정 달성치 마이너스 보정)
- 피크닉 양도 (F2 보류 영역 — D20 흐름 확장)
- 포만 0 = 마음대로 행동 못함 (룰북 98 — 행동 제한 메커니즘)

F2는 *각 영역별 분량·가치 평가 후 선택적 진행*. F2 안에 여러 STEP 분할 가능.

### 의도적 미선택 (F 트랙 외)

- 시나리오 횟수 트래커 (피크닉 2회 제한) — *시나리오* 단위 인프라 별도 필요
- record UI — 게임 외부 트랙
- 마법 효과 다양화 (D30 의도적 미선택, 재평가 필요 시 별도 트랙)
